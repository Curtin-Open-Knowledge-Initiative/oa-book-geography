...
* OA books as a group have {{ text_data.oa_times_more_downloads }} 10 times more downloads than non-OA books and {{ text_data.oa_times_more_citations }} 2.4 times more citations overall.
* There is higher (at least {{ text_data.oa_minimum_times_more }} 2.7-fold) usage (via downloads) of OA books across every stratum in our sample. That is for every type of book, every discipline, and each of the three years of publication in the sample, OA books show more usage than their non-OA comparison groups. This holds for every month after publication and for alternate categories such as imprints.
...
Overall, we find OA books having to reach more countries globally than non-OA books. Figure 3 also presented some evidence that OA books see improved usage for traditionally under-served countries and low to middle income countries. To explore this in more detail we focus on the anonymised downloads, which are solely attributed to OA books (as non-OA books can only be accessed if subscribed to,  i.e., logged access). In particular, we can examine usage in countries that do not otherwise have access to Springer Nature books in digital formats. Usage of OA books was identified in {{ text_data.countries_with_oa_but_not_nonoa_usage }} 67 countries that recorded zero usage of the non-OA books in the data set. Of the 67 countries where only OA books recorded usage, {{ text_data.new_countries_in_africa }}24 countries were from Africa. Usage of OA books from countries that do not otherwise purchase Springer Nature non-OA ebook titles totalled {{ text_data.new_countries_total_downloads }}145,353 downloads, representing {{ text_data.new_countries_downloads_pc }}1% of the total anonymous usage of OA titles.

![](anon_where_no_logged.png)

_Figure 4: Anonymous usage from countries with no logged usage._ A total of {{ text_data.countries_with_oa_but_not_nonoa_usage }}67 countries with zero logged 
usage show anonymous usage with a total of {{ text_data.new_countries_total_downloads }}145,353 downloads representing {{ text_data.new_countries_downloads_pc }}1% of the total anonymous 
usage. Of those countries, {{ text_data.new_countries_in_africa }}24 were from Africa, with others mostly in the Middle East and southeast 
...
At the other end of the scale there is no obvious pattern in the countries that show a higher level of logged usage than anonymous usage for OA books. This list includes Turkey, Liechtenstein and Moldova, where logged and anonymous usage are quite close (Germany has almost precisely the same amount of logged and anonymous usage) and Egypt, Suriname and Taiwan where logged usage is higher than anonymous usage. Overall these differences will be the result of a complex mixture of usage patterns, internet provision and access, as well as Springer Nature market penetration. Iceland is a substantial outlier with only {{ text_data.iceland_anonymous_downloads}}514 anonymous downloads and nearly {{text_data.iceland_Logged_downloads }}25,000 logged. This is likely due to the SpringerLink subscription held by the Iceland Consortium which provides nationwide access. 
...
OA monograph publishers with a limited number of titles. To our knowledge this is the largest independent 
...
measure the geographic diversity of downloads by following closely the codes available from 
...
Data was processed and visualised using the provided Python code which is available at ###GITHUB###
